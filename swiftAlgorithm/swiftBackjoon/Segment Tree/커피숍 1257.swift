...
func solution1275() {
    ...
    func makeTree(node: Int, start: Int, end: Int) -> Int {
        if start == end {
            ...
            tree[node] = makeTree(node: 2*node, start: start, end: (start + end) / 2) + makeTree(node: 2*node + 1, start: (start + end) / 2 + 1, end: end)
        }
    ...
    func sum( node: Int, start: Int, end: Int, left: Int, right: Int) -> Int {
        ...
        return sum( node: 2 * node, start: start, end: (start + end) / 2, left: left , right: right) + sum(node: 2*node + 1, start: (start + end) / 2 + 1, end: end, left: left, right: right)
    }
    ...
    for i in 0 ..< NQ[1] {
        ...
        update(node: 1, start: 0, end: NQ[0] - 1, index: temp[2] - 1, diff: temp[3] - 1)
    }
}
